...
pipeline. It orchestrates data ingestion from multiple sources (CSV, Parquet,
Excel, HTTP APIs), applies advanced financial transformations, calculates key
performance indicators (KPIs), and generates executive reports.
...
- **Weekly**: Run `make audit-code` to ensure engineering standards are
  maintained.
- **Monthly**: Review and update KPI definitions in
  `config/kpis/kpi_definitions.yaml`.
- **v2.0 Cutover**: Delete `config/LEGACY/` and archived modules as per the
  migration plan.
