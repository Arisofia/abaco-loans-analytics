...
## 1. Objective
Transform the fragmented, multi-version pipeline into a unified, contract-driven, and testable system with full data lineage and observability.
...
1. **Inventory & Sweep**: Finalize deletion of orphaned files identified in Phase 1 audit.
2. **Module Consolidation**: Move `src/` contents to `src/` and resolve import paths.
3. **Contract Implementation**: Implement Great Expectations validation for the Ingest phase.
4. **Dashboard Linkage**: Update Streamlit/Next.js dashboards to pull from versioned artifacts.
