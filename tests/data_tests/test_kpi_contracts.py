...
def test_segment_level_contracts():
    ...
    consumer_par_90, _ = calculate_par_90(consumer)
    sme_par_90, _ = calculate_par_90(sme)

    assert consumer_par_90 == pytest.approx(3.25, rel=0.001)
    assert sme_par_90 == pytest.approx(3.25, rel=0.001)

    consumer_collection, _ = calculate_collection_rate(consumer)
    sme_collection, _ = calculate_collection_rate(sme)
