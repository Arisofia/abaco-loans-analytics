...
echo "Step 1: Updating imports to use src.pipeline.ingestion..."
...
sed -i '' 's/from python\.ingestion import CascadeIngestion/from src.pipeline.ingestion import UnifiedIngestion/g' tests/test_pipeline.py
sed -i '' 's/CascadeIngestion/UnifiedIngestion/g' tests/test_pipeline.py

# scripts/run_data_pipeline.py
sed -i '' 's/from python\.ingestion import CascadeIngestion/from src.pipeline.ingestion import UnifiedIngestion/g' scripts/run_data_pipeline.py
...
grep -r "from src.transformation import\|from python import transformation" --include="*.py" --exclude-dir=.venv --exclude-dir=node_modules 2>/dev/null || echo "No imports found"
echo ""

# Step 3: Delete legacy modules
echo "Step 3: Deleting legacy modules..."
rm -f src/ingestion.py && echo "✓ Deleted src/ingestion.py"
rm -f src/transformation.py && echo "✓ Deleted src/transformation.py"
rm -f src/pipeline/calculation.py && echo "✓ Deleted src/pipeline/calculation.py"

# Rename calculation_v2 to calculation
mv src/pipeline/calculation_v2.py src/pipeline/calculation.py && echo "✓ Renamed calculation_v2.py → calculation.py"
