...
def _status_by_thresholds(value: Optional[Decimal], green: Decimal, yellow: Decimal, direction: str) -> str:
    if value is None:
        return "unknown"
...
class KPICalculator:
    ...
    async def __aenter__(self) -> "KPICalculator":
        self.pool = await asyncpg.create_pool(self.db_uri, min_size=1, max_size=5, command_timeout=30)
        return self
    ...
    async def par_90(self, snapshot_id: str, as_of: date) -> KPIResult:
        ...
        ih = _hash_inputs({"total": str(total_d), "delinquent": str(delinquent_d), "snapshot_id": snapshot_id, "as_of": str(as_of)})
        status = _status_by_thresholds(rate, green=Decimal("3"), yellow=Decimal("5"), direction="lower_is_better")
        return KPIResult(
            kpi_key="risk.par_90",
            value_num=rate,
            unit="percent",
