...
def test_apply_deduplication_config(tmp_path, base_config):
    ...
    cfg["pipeline"]["phases"]["ingestion"]["deduplication"] = {"enabled": True, "key_columns": ["loan_id"]}
...
def test_validation_strict_raises(tmp_path):
    cfg = {"pipeline": {"phases": {"ingestion": {"validation": {"required_columns": ["loan_id", "total_receivable_usd"], "strict": True}}}}}
    ui = UnifiedIngestion(cfg)
