...
def make_loans(dates, disburse_dates=None, maturity_dates=None, dpd_vals=None, balances=None):
    data = {"dpd": dpd_vals or [0] * len(dates), "outstanding_balance": balances or [10] * len(dates)}
...
def test_measurement_date_column_takes_precedence(base_config):
    loans = pd.DataFrame({"days_past_due": [5], "outstanding_balance": [50], "as_of_date": ["2022-12-12"]})
    cfg = base_config.copy()
