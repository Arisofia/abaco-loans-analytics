...
class KPICatalogProcessor:
    ...
    def build_loan_month(
        ...
        for me in month_ends:
            ...
            if not temp.empty:
                agg = (
                    temp.groupby("loan_id")["disbursement_amount"]
                    .sum()
                    .reset_index()
                )
                agg["month_end"] = me
    ...
    def get_customer_types(self) -> pd.DataFrame:
        ...
        summary = (
            ...
            .agg({"customer_id": "nunique", "disbursement_amount": "sum"})
            .reset_index()
        )

        summary.rename(
            columns={"customer_id": "unique_customers"}, inplace=True
        )
        return summary
    ...
    def get_payor_concentration(self) -> pd.DataFrame:
        ...
        pagador_col = (
            "pagador" if "pagador" in self.loans.columns else "customer_id"
        )
        df = self.loan_month.merge(
            self.loans[["loan_id", pagador_col]], on="loan_id", how="left"
        )

        summary = (
            df.groupby(["month_end", pagador_col])
            .agg(
